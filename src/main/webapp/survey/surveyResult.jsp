   ...
   <div class="main">
      <div class="container">
         ...
         <div>
            ...
            <div>
               <ul>
                  <c:if test="${dto.getUncomfort1() ne 'null'}">
                     <li>${dto.getUncomfort1()}</li>
                  </c:if>
                  <c:if test="${dto.getUncomfort2() ne 'null'}">
                     <li>${dto.getUncomfort2()}</li>
                  </c:if>
                  <c:if test="${dto.getUncomfort3() ne 'null'}">
                     <li>${dto.getUncomfort3()}</li>
                  </c:if>
                  <c:if test="${dto.getExercise() ne 'null'}">
                     <li>${dto.getExercise()}</li>
                  </c:if>
                  <c:if test="${dto.getActivity() ne 'null'}">
                     <li>${dto.getActivity()}</li>
                  </c:if>
                  <c:if test="${dto.getEating_habits() ne 'null'}">
                     <li>${dto.getEating_habits()}</li>
                  </c:if>
                  <c:if test="${dto.getFavorite_food() ne 'null'}">
                     <li>${dto.getFavorite_food()}</li>
                  </c:if>
                  <c:if test="${dto.getFamily_disease() ne 'null'}">
                     <li>${dto.getFamily_disease()}</li>
                  </c:if>
                  <c:if test="${dto.getSupplements() ne 'null'}">
                     <li>${dto.getSupplements()}</li>
                  </c:if>
               </ul>
            </div>
         </div>
         <div>
            <input type="button" class="surveyResult" id="btnMain" name="btnMain"
               value="메인으로"> <input type="button" class="surveyResult"
               id="btnPayment" name="btnPayment" value="결제하기">
         </div>
      </div>
      <script>
         document.getElementById("btnMain").onclick = function() {
            location.href = "/toMain.srv"
         }
         document.getElementById("btnPayment").onclick = function() {
            location.href = "/toPayment.srv"
         }
      </script>
