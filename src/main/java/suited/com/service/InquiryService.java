...
public class InquiryService {
	private InquiryDAO dao = new InquiryDAO(); // 사용할 인스턴스를 만들어준다. 
	...
	public HashMap<String, Integer> getRange(int currentPage) throws Exception{
	...
	public HashMap<String, Object> getPageNavi(int currentPage, HashMap<String, String> map) throws Exception{
		// 1. 전체 데이터가 몇개인지 뽑아내는 작업
		int recordTotalCnt = 0;
		
		if(map.get("admin_yn").equals("1")) {
			recordTotalCnt = dao.countAll();
		}else if(map.get("admin_yn").equals("0")) {
			recordTotalCnt = dao.countById(map.get("id"));
		}
